...
namespace SAP.Middleware.Connector.Examples
{
    ...
    public class StepByStepServer
    {
        ...
        //
        public static void ExampleGenericServerStaticDefaultHandler()
        {
            GenericServer(true);
        }

        public static void ExampleGenericServerInstanceDefaultHandler()
        {
            GenericServer(false);
        }

        private static void GenericServer(bool useStaticDefaultHandler)
        {
            ...
            RfcServer server = RfcServerManager.GetServer(NCO_SERVER_CONFIG_NAME, new Type[] { useStaticDefaultHandler ? typeof(ServerFunctionGenericStaticImpl) : typeof(ServerFunctionGenericImpl), typeof(StfcConnectionStaticImpl) });
            ...
            server.Start();
            Console.WriteLine();
            Console.WriteLine("Generic server using {1} default handler started: {0}", server.Parameters.ToString(), useStaticDefaultHandler ? "static" : "instance");
            ...
            public static void StfcConnection(RfcServerContext serverContext, IRfcFunction function)
            {
				...
				if (serverContext.InTransaction) {
					...
					if (requtext.Equals("Aborts")) {
                        ...
                        //TODO: throw new RfcAbapRuntimeException(errorMessage);
                    }
                }
        ...
        // It implements one function, namely STFC_CONNECTION. If more functions were to be handled
        // more instance methods analogous to the available method could be implemented.
        // 
			...
			public static void RfcRaiseError(RfcServerContext serverContext, IRfcFunction rfcFunction) {
                ...
                switch (methodNumber)
                {
					...
					case 3: //SYSTEM_FAILURE                        
                         //TODO: throw new RfcAbapRuntimeException("I'm very sorry, but something went wrong overhere...");
					...
					case 5: //ABAP class based exception
                        ...
                        //TODO: throw new RfcAbapClassException("NCo3 step-by-step server tutorial message", exceptionObject);
                        throw new RfcAbapException("RAISE_EXCEPTION");
                }
			}
		}
        ...
        // 
        class ServerFunctionGenericStaticImpl
        {
        ...
        //
        // This class represents a generic function handler that can be used by a server to handle any kind of (stateless or stateful) calls.
        // It implements a default handler that receives every function not handled by any other handler available to the server.
        // 
        class ServerFunctionGenericImpl
        {
            [RfcServerFunction(Default = true)]
            public void GenericHandler(RfcServerContext serverContext, IRfcFunction rfcFunction)
            {
                MethodBase method = MethodInfo.GetCurrentMethod();
                Console.WriteLine();
                Console.WriteLine("Method {2}.{0} processing RFC call {1}", method.Name, rfcFunction.Metadata.Name, method.DeclaringType.ToString());
                ...
                for (int i = 0; i < rfcFunction.Metadata.ParameterCount; ++i)
                {
                    if (0 != (rfcFunction.Metadata[i].Direction & RfcDirection.IMPORT))
                    { //  || rfcFunction.Metadata[i].Direction == RfcDirection.CHANGING) {
                        switch (rfcFunction.Metadata[i].DataType)
                        {
                            case RfcDataType.STRUCTURE:
                                Console.WriteLine("Received structure of name {0} and type {1}", rfcFunction.Metadata[i].Name, rfcFunction.Metadata[i].ValueMetadataAsStructureMetadata.Name);
        ...
        class MyUnitIDHandler//TODO : IUnitIDHandler
        {
            // Only for tests. In real life scenarios, use a database to store the transaction state!
